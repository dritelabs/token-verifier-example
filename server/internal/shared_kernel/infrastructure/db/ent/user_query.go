...
//
//	client.User.Query().
//		GroupBy(user.FieldDefaultShippingAddressID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.User.Query().
//		Select(user.FieldDefaultShippingAddressID).
//		Scan(ctx, &v)
...
func (uq *UserQuery) loadProfile(ctx context.Context, query *ProfileQuery, nodes []*User, init func(*User), assign func(*User, *Profile)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_profile" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
